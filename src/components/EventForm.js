...
const EventForm = () => {
  ...
  const deleteAllOperationLogs = e => {
    e.preventDefault()
    const result = window.confirm('全てのログ操作を本当に削除しても良いですか?')
    if(result){
      dispatch({
  ...
  return (
    <>
      ...
      <form>
        ...
        <button className="btn btn-danger" onClick={deleteAllOperationLogs} disabled={state.operationLogs.length === 0}>全てのログ操作を削除する</button>
      </form>
    </>
  )
}
